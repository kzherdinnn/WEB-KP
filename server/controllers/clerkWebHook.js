...
const clerkWebHooks = async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Verifikasi webhook gagal" });
    }
    ...
    if (type === "user.created" || type === "user.updated") {
      ...
      if (!data.email_addresses?.[0]?.email_address) {
        ...
        return res.status(200).json({
          success: true,
          message: "Event diterima tapi data tidak lengkap",
        });
      }
      ...
      // ✅ AUTO-PROMOTE ADMIN: Cek apakah email ada di list admin
      ...
      if (isAdminEmail) {
        userData.role = "admin";
        console.log(`🔑 User ini adalah ADMIN (email terdaftar)`);
      }

      if (type === "user.created") {
        await userModel.create(userData);
        console.log(
          `Pengguna BERHASIL dibuat di database. ID: ${userData._id}`,
        );
        if (isAdminEmail) {
          console.log(`👨‍💼 User otomatis dipromosikan ke ADMIN!`);
        }
        ...
        console.log(
          `Pengguna BERHASIL diperbarui di database. ID: ${userData._id}`,
        );
        if (isAdminEmail) {
          console.log(`👨‍💼 User otomatis dipromosikan ke ADMIN!`);
        }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Webhook berhasil diproses" });
  } catch (error) {
    console.error("Error dalam handler webhook:", error.message);
    return res
      .status(500)
      .json({ success: false, message: "Terjadi kesalahan internal" });
  }
};
